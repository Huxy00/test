...
def model_hxy():
    ...
    feat_columns = ['ID', 'time', 'people_index', 'day', 'weekday', 'hour', 'area', 'area_type', 'move_index',
                    'hour_in_index', 'hour_end_index']
    ...
    def get_feat(label, history):
        ...
        label['area_type_encode'] = label['area_type'].map(
            dict(history_1.groupby(['area_type']).mean()['people_index']))
        ...
        for i in range(t):
            ...
            # 最近2天减最远一天
            tmp['pi_2-7'] = history[history['day'] == day_ - 1]['people_index'].values - \
                            history[history['day'] == day_ - 6]['people_index'].values
...
def rule(df_train, df_test):
    ...
    # 星期日
    m = 17
    y_his_1_1 = 0.1 * history_1[history_1['day'] == m - 1]['people_index'].values + 0.8 * \
                history_1[history_1['day'] == m]['people_index'].values + 0.1 * history_1[history_1['day'] == m + 1][
                    'people_index'].values
    # 星期一
    m = m + 1
    y_his_1_2 = 0.1 * history_1[history_1['day'] == m - 1]['people_index'].values + 0.8 * \
                history_1[history_1['day'] == m]['people_index'].values + 0.1 * history_1[history_1['day'] == m + 1][
                    'people_index'].values
    # 星期二
    m = m + 1
    y_his_1_3 = 0.1 * history_1[history_1['day'] == m - 1]['people_index'].values + 0.8 * \
                history_1[history_1['day'] == m]['people_index'].values + 0.1 * history_1[history_1['day'] == m + 1][
                    'people_index'].values
    # 星期三
    m = m + 1
    y_his_1_4 = 0.1 * history_1[history_1['day'] == m - 1]['people_index'].values + 0.8 * \
                history_1[history_1['day'] == m]['people_index'].values + 0.1 * history_1[history_1['day'] == m + 1][
                    'people_index'].values
    # 星期四
    m = m + 1
    y_his_1_5 = 0.1 * history_1[history_1['day'] == m - 1]['people_index'].values + 0.8 * \
                history_1[history_1['day'] == m]['people_index'].values + 0.1 * history_1[history_1['day'] == m + 1][
                    'people_index'].values
    # 星期五
    m = m + 1
    y_his_1_6 = 0.1 * history_1[history_1['day'] == m - 1]['people_index'].values + 0.8 * \
                history_1[history_1['day'] == m]['people_index'].values + 0.1 * history_1[history_1['day'] == m + 1][
                    'people_index'].values
    # 星期六
    m = m + 1
    y_his_1_7 = 0.1 * history_1[history_1['day'] == m - 1]['people_index'].values + 0.8 * \
                history_1[history_1['day'] == m]['people_index'].values + 0.1 * history_1[history_1['day'] == m + 1][
                    'people_index'].values
    # 星期日
    m = 23
    y_his_2_1 = 0.1 * history_2[history_2['day'] == m - 1]['people_index'].values + 0.8 * \
                history_2[history_2['day'] == m]['people_index'].values + 0.1 * history_2[history_2['day'] == m + 1][
                    'people_index'].values
    # 星期一
    m = m + 1
    y_his_2_2 = 0.1 * history_2[history_2['day'] == m - 1]['people_index'].values + 0.8 * \
                history_2[history_2['day'] == m]['people_index'].values + 0.1 * history_2[history_2['day'] == m + 1][
                    'people_index'].values
    # 星期二
    m = m + 1
    y_his_2_3 = 0.1 * history_2[history_2['day'] == m - 1]['people_index'].values + 0.8 * \
                history_2[history_2['day'] == m]['people_index'].values + 0.1 * history_2[history_2['day'] == m + 1][
                    'people_index'].values
    # 星期三
    m = m + 1
    y_his_2_4 = 0.1 * history_2[history_2['day'] == m - 1]['people_index'].values + 0.8 * \
                history_2[history_2['day'] == m]['people_index'].values + 0.1 * history_2[history_2['day'] == m + 1][
                    'people_index'].values
    # 星期四
    m = m + 1
    y_his_2_5 = 0.1 * history_2[history_2['day'] == m - 1]['people_index'].values + 0.8 * \
                history_2[history_2['day'] == m]['people_index'].values + 0.1 * history_2[history_2['day'] == m + 1][
                    'people_index'].values
    # 星期五
    m = m + 1
    y_his_2_6 = 0.1 * history_2[history_2['day'] == m - 1]['people_index'].values + 0.8 * \
                history_2[history_2['day'] == m]['people_index'].values + 0.1 * history_2[history_2['day'] == m + 1][
                    'people_index'].values
    # 星期六
    m = m + 1
    y_his_2_7 = 0.1 * history_2[history_2['day'] == m - 1]['people_index'].values + 0.9 * \
                history_2[history_2['day'] == m]['people_index'].values
    # 最近
    y_near = 0.5 * history_2[history_2['day'] == 29]['people_index'].values + 0.3 * history_2[history_2['day'] == 28][
        'people_index'].values + 0.2 * history_2[history_2['day'] == 27]['people_index'].values
    ...
    for i in range(9):
        ...
        y_trends[i][(y_trends[i] < 0.7) | (y_trends[i] > 1.2)] = (y_trends[i][
            (y_trends[i] < 0.7) | (y_trends[i] > 1.2)]) ** 0.5
        # 140
...
def get_sub():
    sub = pd.read_csv("./data/test_submit_example.csv", header=None,
                      names=['ID', 'time', 'people_index'])
    hxy, df_train, df_test = model_hxy()
    hxy_rule = rule(df_train, df_test)
    sub.iloc[:, 2] = hxy * 0.3 + hxy_rule * 0.7
    return sub
...
if __name__ == '__main__':
    ...
    print(end - start)
    print("---------------------------------")
    print(np.mean(hxy_result['people_index']))
    print("---------------------------------")
    print(np.max(hxy_result['people_index']))
